...
import { filterArticles, useAllPlatformArticles } from "@/hooks/useArticles";
...
function ErrorDisplay({
  errors,
  onRetry,
}: { errors: Record<string, string>; onRetry: () => void }) {
    ...
    <div className="mb-6 p-4 bg-red-50 border border-red-200 rounded-lg">
      ...
      <div className="space-y-1 mb-3">
        {errorEntries.map(([platform, message]) => (
          <p key={`${platform}-${message}`} className="text-sm text-red-700">
            {platform}: {message}
          </p>
        ))}
      </div>
...
export default function Articles() {
  ...
  const { featuredArticles, otherArticles } = useMemo(() => {
    ...
    if (showApiArticles) {
      allArticles = [...zennArticles, ...qiitaArticles, ...noteArticles];
    } else {
      allArticles = [...staticFeaturedArticles, ...staticOtherArticles];
    }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-sky-50 via-white to-blue-50">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex bg-white rounded-lg p-1 border">
                    ...
                    >
                      API記事
                      {loading && (
                        <RefreshCw className="ml-2 h-3 w-3 animate-spin inline" />
                      )}
                    </button>
                  </div>
                </div>
      ...
      {/* Error Display */}
      {showApiArticles && Object.keys(errors).length > 0 && (
        <section className="px-4">
          <div className="container mx-auto">
            <ErrorDisplay errors={errors} onRetry={refresh} />
          </div>
        </section>
      )}
