...
export async function fetchZennArticles(): Promise<Article[]> {
  try {
    ...
    const apiError: ApiError = {
      message: error instanceof Error ? error.message : "Zenn記事の取得に失敗しました",
      status: error instanceof Error && 'status' in error ? Number(error.status) : undefined,
      platform: "Zenn"
    };
...
export async function fetchQiitaArticles(): Promise<Article[]> {
  try {
    ...
    const apiError: ApiError = {
      message: error instanceof Error ? error.message : "Qiita記事の取得に失敗しました",
      status: error instanceof Error && 'status' in error ? Number(error.status) : undefined,
      platform: "Qiita"
    };
...
export async function fetchNoteArticles(): Promise<Article[]> {
  ...
  console.warn("Note API is not available. Consider using RSS feed or manual updates.");
  return [];
}
 ...
 * 特定のプラットフォームの記事のみを取得
 */
export async function fetchArticlesByPlatform(platform: "Zenn" | "Qiita" | "Note"): Promise<Article[]> {
  switch (platform) {
    case "Zenn":
